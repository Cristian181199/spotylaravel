<x-contenido>
    ...
    <form action="{{ route('temas.update', [$tema], false) }}" method="POST">
        @csrf
        @method('PUT')
        <h1 class="text-center text-2xl font-bold text-gray-500 mb-10">EDITAR TEMA</h1>
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                @endphp
                <input type="text" placeholder="00:00" name="duracion" id="duracion" class="ml-2 outline-none py-1 px-2 text-md border-2 rounded-md @error('duracion') border-red-500 @enderror" value="{{ old('duracion', $duracion->format('%H:%I:%S')) }}"/>
                @error('duracion')
                    <span class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">
                        {{ $message }}
                    </span>
